...
getattachment_bp = Blueprint('getattachment', __name__, url_prefix="/getattachment")

@getattachment_bp.route("/", methods=["GET"])
@access_required(4)
def getattachment():
    data=request.args
    ...
    if token_dict:
        ...
        if userType <= 2:
            ...
            # return "ok"
        if file_data_buffer:
            return send_file(BytesIO(file_data_buffer), attachment_filename="atch.pdf", as_attachment = False)
